...
cp -a /usr/local/etc/xray/self_signed_cert.pem /voidvpn/xray/xray.crt
cp -a /usr/local/etc/xray/self_signed_key.pem /voidvpn/xray/xray.key
...
if "$HOME"/.acme.sh/acme.sh --issue -d "${domain}" --webroot "/var/www/html" -k ec-256 --force > /dev/null 2>&1; then
	...
	sleep 1
	if "$HOME"/.acme.sh/acme.sh --installcert -d "${domain}" --fullchainpath /voidvpn/xray/xray.crt --keypath /voidvpn/xray/xray.key --reloadcmd "systemctl restart xray" --ecc --force > /dev/null 2>&1; then
